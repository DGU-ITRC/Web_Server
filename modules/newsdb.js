...
async function insertNews(thumb, title, content, count, created, url) {
  return new Promise((resolve, reject) => {
    // 먼저 중복된 URL이 있는지 확인
    db.get('SELECT * FROM news WHERE url = ?', [url.trim().toLowerCase()], (err, row) => {
      if (err) {
      ...
      if (!row) {
        ...
        stmt.run(thumb, title, content, count, created, url.trim().toLowerCase(), (err) => {
          if (err) {
...
async function getNews(orderBy = 'created', limit = 20) {
  return new Promise((resolve, reject) => {
    //SQL injection 방지
    const validOrderBy = orderBy === 'created' || orderBy === 'count' ? orderBy : 'created';
    const subOrderBy = validOrderBy === 'created' ? 'count' : 'created';

    const query = `SELECT * FROM news ORDER BY ${validOrderBy} DESC, ${subOrderBy} DESC LIMIT ?`;
